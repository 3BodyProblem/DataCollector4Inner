...
int QuotationRecorder::Record( unsigned short usMessageNo, unsigned short usFunctionID, const char* pszData, unsigned int nLen )
{
	...
	m_oDataFile.write( (char*)&usMessageNo, sizeof(usMessageNo) );
	m_oDataFile.write( (char*)&usFunctionID, sizeof(usFunctionID) );
	m_oDataFile.write( pszData, nLen );
	nLen += (sizeof(unsigned short) * 2);
...
int QuotationRecover::Read( unsigned short& usMessageNo, unsigned short& usFunctionID, char* pszData, unsigned int nLen )
{
	if( NULL == pszData || 0 == nLen )
	{
		return -1;
	...
	m_oDataFile.read( (char*)&usMessageNo, sizeof(usMessageNo) );
	m_oDataFile.read( (char*)&usFunctionID, sizeof(usFunctionID) );
	m_oDataFile.read( pszData, oIndex.nLen-(sizeof(unsigned short)*2) );
